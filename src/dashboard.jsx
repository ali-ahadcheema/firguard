...
<p className="text-sm"><span className="font-medium text-white">Total ALerts</span><br/><span className="text-3xl text-red-600">{total}</span><br/><span className="text-red-600 flex items-center gap-1"><FaChevronUp size={16} color="red"/> 12% <span className="text-white">from last week</span> </span></p>
    </div>
      <div className=" flex items-center gap-2  bg-[#1F2937] h-24 w-72 border-[1px] border-[#374151] rounded-lg">
...
<button onClick={()=>usenavi("/camera")} className="bg-[#0F172A] justify-center h-8 w-48 flex items-center gap-1.5 text-white border-0 rounded cursor-pointer">
 ...
 {data.filter(item=>item.status==="fire detected").slice(0,3).map((item)=>(
  <div key={item.id} className="flex items-center h-17 border-b-[1px] border-gray-400 w-full justify-around hover:scale-105 transition duration-150">
    <div className="flex items-center gap-2">
      <img className="h-8 w-8" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRv7MO6GmOypVwyo48h8Za97ChdfJjaBwB3pg&s"/>
      <p className="text-sm text-white">
        <span className="font-medium text-white">Fire detected</span><br/>
        {item.time}
      </p>
    </div>
